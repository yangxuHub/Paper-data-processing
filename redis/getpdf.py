...
# headers 保持与服务器的会话连接'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36 Edg/96.0.1054.62',
# headers = {
...
def getPaperPdf(url, doi):
    ...
    file_name = doi.split('/')[-2] + '%' + doi.split('/')[-1] + '.pdf'
...
if __name__ == '__main__':
    sci_Hub_Url = "https://arxiv.org/pdf/"#"https://sci-hub.ren/"
    # doi = input("doi:")
    with open('arxiv.csv', 'r', encoding='utf-8')as f:
        doi_lists = list(csv.reader(f))
    num = 1
    for doi in doi_lists[:39800]:
        if doi[0] is None:
            print('DOI IS NOME...')
        else:
            print(doi[0])
            paper_url = sci_Hub_Url + doi[0]
            print(paper_url)
            try:
                getPaperPdf(paper_url, doi[0])  # 通过文献的url下载pdf
                num = num + 1
                print(num)
            except Exception:
